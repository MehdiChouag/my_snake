...
// Last update Sat Apr  5 20:08:31 2014 Mehdi Chouag
//
...
void		Menu::initSDL()
{
  ...
  if (_x == 1920 && _y >= 960)
    _screen = Lib::xSDL_SetVideoMode(_x, _y, 32, SDL_HWSURFACE | SDL_FULLSCREEN);
  else
  ...
  if (SDL_NumJoysticks() >= 1)
    {
      ...
      if (_joystick == NULL)
	std::cerr << "Can't init the joystick" << std::endl;
    }
